...
uint8_t		coilVal;
uint8_t		inputVal;
uint8_t statusModbus = MODBUS_OK;

uint16_t ESP32MsgHandler_ReadRegister(uint8_t type, uint16_t addr) {
    switch (type) {
        case REG_TYPE_HOLDING:
        	statusModbus = Modbus_ReadInputRegisters(&hmodbus, SlaveID, addr, 1, &inputReg);
            return inputReg;
        case REG_TYPE_INPUT:
        	statusModbus = Modbus_ReadHoldingRegisters(&hmodbus, SlaveID, addr, 1, &holdingReg);
            return holdingReg;
        case REG_TYPE_COIL:
        	statusModbus = Modbus_ReadCoils(&hmodbus, SlaveID, addr, 1, &coilVal);
            return coilVal ? 1 : 0;
        case REG_TYPE_DISCRETE:
        	statusModbus = Modbus_ReadDiscreteInputs(&hmodbus, SlaveID, addr, 1, &inputVal);
            return inputVal ? 1 : 0;
        default:
            return 0;
    }
...
uint8_t ESP32MsgHandler_MultipleReadRegister(uint8_t type, uint16_t addr, uint8_t quantity, uint8_t *buff) {
    switch (type) {
        case REG_TYPE_HOLDING:
        	statusModbus = Modbus_ReadHoldingRegisters(&hmodbus, SlaveID, addr, quantity, holdingRegisters);
        	if(statusModbus == MODBUS_OK){
				for(int i = 0; i < quantity; i = i+2){
					int j = i / 2;
					buff[i+3] = (holdingRegisters[j] >> 8);
					buff[i+4] = holdingRegisters[j];
				}
				return 0;
        	}
        	return statusModbus;
        case REG_TYPE_INPUT:
        	statusModbus = Modbus_ReadInputRegisters(&hmodbus, SlaveID, addr, quantity, inputRegisters);
        	if(statusModbus == MODBUS_OK){
        	...
        	return statusModbus;
        case REG_TYPE_COIL:
        	statusModbus = Modbus_ReadCoils(&hmodbus, SlaveID, addr, quantity, coils);
        	if(statusModbus == MODBUS_OK){
        	...
        	return statusModbus;
        case REG_TYPE_DISCRETE:
        	statusModbus = Modbus_ReadDiscreteInputs(&hmodbus, SlaveID, addr, quantity, discreteInputs);
        	if(statusModbus == MODBUS_OK){
				for(int i = 3; i < quantity; i++){
...
uint8_t ESP32MsgHandler_WriteRegister(uint8_t type, uint16_t addr, uint16_t value) {
    switch (type) {
        case REG_TYPE_HOLDING:
        	statusModbus = Modbus_WriteSingleRegister(&hmodbus, SlaveID, addr, value);
        	return statusModbus;
        case REG_TYPE_COIL:
        	statusModbus = Modbus_WriteSingleCoil(&hmodbus, SlaveID, addr, value);
        	return statusModbus;
